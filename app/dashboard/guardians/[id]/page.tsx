...
/**
 * Edit Guardian Page (Phase 20)
 * Edit existing guardian/parent with contact details and student linking
 *
 * Features:
 * - Load and display existing guardian data
...
export default function EditGuardianPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!user?.tenantId || !guardianId) return;

      try {
        // Load guardian data
        const guardianDoc = await getDoc(doc(db, 'users', guardianId));
        if (!guardianDoc.exists() || guardianDoc.data().tenantId !== user.tenantId) {
        ...
        setFormData({
          ...
          phone2: guardianData.phone2 || '',
          address: guardianData.address || '',
          occupation: guardianData.occupation || '',
          relationshipType: guardianData.relationshipType || 'father',
          isPrimary: guardianData.isPrimary || false,
          isEmergencyContact: guardianData.isEmergencyContact || false,
          contactPreferences: guardianData.contactPreferences || {
            email: true,
            sms: false,
            call: true,
          },
        ...
        // Set currently linked students
        const linkedStudentIds = studentsData
          .filter(s => s.guardianIds?.includes(guardianId))
          .map(s => s.id);
        setSelectedStudents(linkedStudentIds);
        setOriginalStudents(linkedStudentIds);
    ...
    loadData();
  }, [user?.tenantId, guardianId, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Remove guardian from students no longer linked
      ...
      addedStudents.forEach(studentId => {
        ...
        if (student) {
          const studentRef = doc(db, 'students', studentId);
          const updatedGuardianIds = [...new Set([...(student.guardianIds || []), guardianId])];
          batch.update(studentRef, {
            guardianIds: updatedGuardianIds,
            updatedAt: serverTimestamp(),
          });
      ...
      setErrors({ submit: 'Failed to update guardian. Please try again.' });
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Guardian</h1>
          <p className="text-gray-600 mt-1">Update guardian/parent information and student links</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          <Button
            ...
            className="flex-1"
          >
            {saving ? 'Updating Guardian...' : 'Update Guardian'}
          </Button>
          <Button
            type="button"
            variant="outline"
